...
Suppose the predictions $\boldsymbol{s}_1, \ldots, \boldsymbol{s}_K$ span the vector space $\mathbb{R}^K$. We call this condition ``star independence''.  In that case the prediction $\hat{\boldsymbol{p}}$ of the method  can be uniquely expressed as a linear combination of $\boldsymbol{s}_i(x)$. 
