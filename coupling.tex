...
A path of length $s$ in $G$ is a sequence of edges 
$(C_{i_1}, C_{i_2}), (C_{i_2}, C_{i_3}), \ldots,  (C_{i_{s}}, C_{i_{s+1}})$. 
A cycle is a path with length $s\geq 3$ such that all vertices $C_{i_1}, \ldots, C_{i_s}$ are distinct and the starting and the ending vertices coincide, i.e., $i_1 = i_{s+1}$. A tree is a graph without a cycle.
A spanning tree is a tree for which there is a path between any pair of vertices $C_i$ and $C_j$. Such a path is necessarily unique \cite[Theorem 4.1]{harary1969graph}. 
