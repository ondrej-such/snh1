...
%If we have a  binary classifier predicting distribution $D \propto (p, p')$ on two classes $C_i$ and $C_j$, then we define new distribution $\pi_{ij}(D)$ by requiring
%$$
%\pi_{ij}(D) \propto \biggl(p \frac{P'_i}{P_i},p' \frac{P'_j}{P_j}\biggr).
%$$
Let $\boldsymbol{R}=(r_{ij})$ be the matrix of binary predictions, where $r_{ij}+r_{ji}=1$. We denote by $\boldsymbol{R}^\pi=(r^\pi_{ij})$ the $K\times K$ matrix with entries satisfying 
$$
