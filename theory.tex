...
Let us recall the formula. Suppose we change priors from $\Pi = (P_1, \ldots, P_k)$ to $\Pi'= (P'_1, \ldots, P'_K)$. 
...
Let us make this explicit. Let $\boldsymbol{v}$ be a coupling method. Denote by  $\pi$ the change of priors $\pi:\Pi \rightarrow \Pi'$. 
Denote $B = B_{ij}$ the binary classifier  predicting distribution $D \propto (p, p')$ on two classes $C_i$ and $C_j$ before change of priors. 
After the change $\pi$ the equation \eqref{eq:changePrior} yields the following expression for the probability of class $C_i$:
% $$
% \pi_{ij}(D) \propto \biggl(p \frac{P'_i}{P_i},p' \frac{P'_j}{P_j}\biggr).
% $$
\begin{align}
p_B^\pi(C_i) = \frac{p \frac{P'_i}{P_i}}{p \frac{P'_i}{P_i} + p' \frac{P'_j}{P_j}}.
\end{align}
We denote by $\boldsymbol{R}^\pi$ the $K\times K$ matrix with entries $p_{B_{ij}}^\pi(C_i)$ in $i$-th row and $j$-the column.
