...
\item  the three arboreal methods $\boldsymbol{s}_1, \boldsymbol{s}_2, \boldsymbol{s}_3$ when $(a_1,a_2,a_3)$ equals to respectively
   $(1,0,0)$, $(0,1,0)$ and $(0,0,1)$.
\end{itemize}

We do not know if this family possessess any convexity properties which would allow us to find the optimal classifier in the family.
Therefore we select a finite subset of combinations for which $a_1, a_2, a_3$ are nonnegative rational numbers summing to one with denominator equal to 50. 
...
We visualize the results in \ref{fig:par-bc7}. The results are split according to the dataset, 
...
Secondly, we turn out attention whether we would be \emph{able to find} the 
...
\caption{Comparison of accuracy for parametric Bayes covariant methods \eqref{eq:family} vs. Wu--Lin--Weng's method. Accuracy is based on repeated five-fold cross-validation to avoid parameter selection bias, and is evaluated on the subsets where normal method $\boldsymbol{e}_3$ was outperformed by Wu-Lin-Weng's method.}
\label{fig:par-bc}
\end{figure}
