...
Let us now turn to recommendation to practitioners on how to handle a change of priors in one-vs-one probabilistic models. It is possible to choose any of the following approaches:

\begin{itemize}
\item[1.] One can treat the decision on when to apply a change of priors---whether before or after coupling---as a boolean hyperparameter and use standard machine learning techniques to find the optimal value, or 
\item[2.] one may opt for a Bayes covariant method, of which there are many. We note that the number of arboreal coupling methods equals to the number of spanning trees on $K$ vertices which is $K^{K-2}$ (\cite{cayley1878theorem}). Additionally, new methods can be constructed by taking weighted combinations as described in Section \ref{sec:bc2}.
\end{itemize}
%\subsection{Properties of coupling methods}

Finally,  for convenience we  summarize properties of parameterless coupling methods discussed in our work.  Table \ref{tab:summaryCoupling} concisely lists the relevant properties enjoyed by each method. Note that we have split the desideratum ``simplicity of computation'' into three different aspects:
\begin{itemize}
