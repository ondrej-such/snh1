...
\begin{itemize}
\item[1.] One can treat the decision on when to apply change of priors---whether before or after coupling---as a boolean hyperparameter and use standard machine learning techniques to find the optimal value, or 
\item[2.] one may opt for a Bayes covariant method, of which there are many. We note that the number of arboreal coupling methods corresponds to the number of spanning trees on $K$ vertices which is $K^{K-2}$ (\cite{cayley1878theorem). Additionally, new methods can be constructed by taking weighted combinations as described in Section \ref{sec:bc2}.
\end{itemize}
