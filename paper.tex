...
Let $\pi$ be a change of priors from $(P_1, \ldots, P_K)$  to $(P'_1, \ldots, P'_K)$. Let $o_l$ denotes the odds of classes $C_i$ and $C_j$ predicted by coupling method $\boldsymbol{v}_l$. Then the odds of $C_i$ and $C_j$ predicted by $(\bigoplus_i a_i \boldsymbol{v}_i)^\pi$ are 
\begin{align}
	\biggl(\bigoplus_i a_i \boldsymbol{v}_i\biggr)^\pi(\boldsymbol{R}) & = \frac{P'_i}{P_i} \frac{P_j}{P'_j} \biggl(\bigoplus_i a_i \boldsymbol{v}_i\biggr)(\boldsymbol{R}) \\
	&= \frac{P'_i}{P_i} \frac{P_j}{P'_j} \prod_l o_l^{a_l}
\end{align}

Let us now use Bayes covariant of $\boldsymbol{v}_i$. We have
\begin{align}
	\biggl(\bigoplus_i a_i \boldsymbol{v}_i\biggr)(\boldsymbol{R^\pi}) & =  \biggl(\bigoplus_i a_i \boldsymbol{v}_i^\pi\biggr)(\boldsymbol{R}) \\
	&= \frac{P'_i}{P_i} \frac{P_j}{P'_j} \prod_l o_l^{a_l}
\end{align}

It follows that the odds of $C_i$ to $C_j$ for $\bigoplus_i a_i \boldsymbol{v}_i$ are 
\begin{align}
\biggl(\frac{P'_i}{P_i} \frac{P_j}{P'_j} \biggr)^{\sum a_i} \prod_l o_l^{a_l}.
\end{align}
Clearly, under the assumption $\sum_i a_i = 1$, the classifier $\bigoplus_i a_i \boldsymbol{v}_i$ satisfies $BC_{ij}$ and it follows that it is Bayes covariant.
\end{proof}
