...
\end{prop}
\begin{proof}
	Given a multiclass distribution $\boldsymbol{p} = (p_1, \ldots,p_K)$ on $K$ classes, let $\alpha_{ij}(\boldsymbol{p})$ denotes the odds of $C_i$ to $C_j$:
	\begin{align}
		\alpha_{ij}(\boldsymbol{p}) = \frac{p_i}{p_j}
	\end{align}
	
	From \eqref{eq:bt1} we can deduce the odds of classes $C_j$ to $C_i$ as
	\begin{align}
	\alpha_{ij}(\boldsymbol{p}^\Pi) = \frac{p_j^\Pi(x)}{p_j^\Pi(x)} = \frac{1}{p^\Pi_{ij}(x)}- 1.
	\end{align}
